...
def run_reverse_proxy():
    if not config.get("chatgpt_paid", False):
        ...
        logger.error("Please set chatgpt_paid to true in config.conf and restart the server.")
        exit(1)

    proxy_path = config.get("reverse_proxy_binary_path", None)
    if not proxy_path:
        logger.error("You need to set the reverse proxy binary path in config.conf!")
        exit(1)
