...
BASE_URL = config.get("CHATGPT_BASE_URL") or "https://api.openai.com/v1/chat/"
...
class Chatbot:
    ...
    def __init__(
            self,
            api_key: str,
            engine: str = config.get("GPT_ENGINE") or "gpt-3.5-turbo",
            proxy: str = None,
            timeout: float = None,
            max_tokens: int = None,
    ...
    def transfer_message_to_conversation(
        ...
        """
        transfer a message to the conversation
        """
        if self.conversation.get(chat_id, None) is None:
            self.conversation[chat_id] = []
        is_new = True
        for content in self.conversation[chat_id]:
            if content['content'] == message:
                is_new = False
        if is_new:
            self.conversation[chat_id].append({"role": role, "content": message})
    ...
    async def ask_async(
        ...
        """
        response = ""
        async for res in self.ask_stream_async(
                prompt=prompt,
                role=role,
                convo_id=convo_id,
                **kwargs,
...
class AsyncChatGptV3Bot(Chatbot):
    def __init__(
            self,
            api_key: str,
            engine: str = os.environ.get("GPT_ENGINE", None) or "gpt-3.5-turbo",
            proxy: str = None,
            timeout: float = None,
            max_tokens: int = None,
            temperature: float = 0.5,
        ...
        super().__init__(
            api_key=api_key,
            engine=engine if engine else "gpt-3.5-turbo",
            proxy=proxy,
            timeout=timeout,
    ...
    async def ask(
        ...
        Ask a question
        """
        async for msg in self.ask_async(prompt, role, convo_id, **kwargs):
            yield msg
    ...
    async def get_conversation(self, convo_id):
        return self.conversation.get(convo_id)

    async def clear_chats(self, convo_id):
        while True:
            if (
                    self.get_token_count(convo_id) > self.truncate_limit
                    and len(self.conversation[convo_id]) > 1
